...
//### Task 1:(Russian)
//Задача состоит в создании класса `Post`, который будет использовать темы в качестве статического класса и обладать несколькими методами с использованием Generics.

//Класс `Post` должен иметь следующие методы:

//1. `createPost`: метод, который принимает в качестве аргументов заголовок, содержание и тему поста, и создает новый пост с указанными параметрами.

//2. `getPostsByTopic`: метод, который принимает в качестве аргумента тему и возвращает список всех постов, относящихся к этой теме.

//3. `getPostsByAuthor`: метод, который принимает в качестве аргумента имя автора и возвращает список всех постов, написанных этим автором.

//4. `getPostsByDate`: метод, который принимает в качестве аргумента дату и возвращает список всех постов, опубликованных в указанную дату.

//5. `deletePost`: метод, который принимает в качестве аргумента идентификатор поста и удаляет его из списка постов.

//Класс `Post` должен использовать темы в качестве статического класса, чтобы можно было легко получать доступ к ним из любого места программы.

//Класс `Post` также должен использовать Generics для обработки различных типов данных, таких как заголовок, содержание и тема поста.

//Пример использования класса `Post`:

//```csharp
//// Создание нового поста
//Post.createPost("Заголовок поста", "Содержание поста", TECHNOLOGY);

//// Получение списка всех постов по теме
//List<Post> postsByTopic = Post.getPostsByTopic(TECHNOLOGY);

//// Получение списка всех постов по автору
//List<Post> postsByAuthor = Post.getPostsByAuthor("Иванов");

//// Получение списка всех постов по дате
